...
func BenchmarkSendRecv(b *testing.B) {
	for _, size := range []int{10, 100, 1000, 10000} {
		b.Run(
			fmt.Sprintf("msgsize=%d", size),
			func(b *testing.B) { benchmarkSendRecvSize(b, size) },
		)
	}
}

var bitbucketLog = func(format string, args ...interface{}) {}

func benchmarkSendRecvSize(b *testing.B, packetSize int) {
	serverPrivateKey := newPrivateKey(b)
	s := NewServer(serverPrivateKey, bitbucketLog)
	defer s.Close()
	...
	client, err := NewClient(key, connOut, brw, bitbucketLog)
	if err != nil {
