...
func (s *Server) Accept(nc Conn, brw *bufio.ReadWriter, remoteAddr string) {
	...
	s.mu.Lock()
	s.accepts.Add(1)              // while holding s.mu for procUniq read on next line
...
func (s *Server) unregisterClient(c *sclient) {
	...
	// so they can drop their route entries to us. (DRPO; issue 150)
...
type sclient struct {
	...
	preferred   bool
	sentTo      map[key.Public]int64 // recipient => rcpt's latest sclient.procUnique (DRPO; issue 150)
