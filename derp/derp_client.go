...
func NewMeshClient(privateKey key.Private, nc net.Conn, logf logger.Logf, meshKey string) (*Client, error) {
	brw := bufio.NewReadWriter(bufio.NewReader(nc), bufio.NewWriter(nc))
	return newClient(privateKey, nc, brw, logf, meshKey)
}

func NewClient(privateKey key.Private, nc Conn, brw *bufio.ReadWriter, logf logger.Logf) (*Client, error) {
	noMeshKey := ""
	return newClient(privateKey, nc, brw, logf, noMeshKey)
}

func newClient(privateKey key.Private, nc Conn, brw *bufio.ReadWriter, logf logger.Logf, meshKey string) (*Client, error) {
	c := &Client{
