...
func TestConfigRecompilation(t *testing.T) {
	...
	// Non-trivial so we hit the darwin code path.
	config := Config{
	...
	// Initial set should error out and leave m.config unset and m.pendingConfig set.
	if err := m.Set(config); err == nil {
		t.Fatalf("Want non-nil error.  Got nil")
	}
	if m.config != nil || m.pendingConfig == nil {
		t.Fatalf("Want nil config and non-nil pendingConfig.  Got config %v, pending %v", m.config, m.pendingConfig)
	}

	f.GetBaseConfigErr = nil

	// Recompilation without an error should succeed and set m.config and unset m.pendingConfig.
	if err := m.RecompileDNSConfig(); err != nil {
		t.Fatalf("Want nil error.  Got err %v", err)
	}
	if m.config == nil || m.pendingConfig != nil {
		t.Fatalf("Want non-nil config and nil pendingConfig.  Got config %v, pending %v", m.config, m.pendingConfig)
	}
}
