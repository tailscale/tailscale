...
type Config struct {
	...
	OnlyIPv6 bool
	// MagicDNSHostsUnrouted is whether MagicDNS host records exist
	// that are served on demand via [resolver.MagicDNSHosts] (so are
	// not listed in Hosts) and are not covered by any Routes entry;
	// that is, the tailnet has MagicDNS names but MagicDNS domain
	// routing is off. It preserves the effect those records had when
	// they were listed in Hosts: hasHostsWithoutSplitDNSRoutes
	// reports true, keeping quad-100 in the OS resolver path so the
	// names still resolve.
