...
func TestTSMPDisco(t *testing.T) {
	t.Run("IPv6DiscoAdvert", func(t *testing.T) {
		var payload [33]byte
		payload[0] = byte(packet.TSMPTypeDiscoAdvertisement)
		discoKey := make([]byte, 32)
		if _, err := rand.Read(discoKey); err != nil {
			t.Fatalf("unable to generate key: %s", err)
		}
		copy(payload[1:], discoKey)

		pkgSrc := netip.MustParseAddr("2001:db8::1")
		pkgDst := netip.MustParseAddr("2001:db8::2")
		iph := packet.IP6Header{
			IPProto: ipproto.TSMP,
			Src:     pkgSrc,
			Dst:     pkgDst,
		}
		tsmpDiscoAdvert := packet.Generate(iph, payload[:])

		var p packet.Parsed
		p.Decode(tsmpDiscoAdvert)

		src, key, ok := p.AsTSMPDiscoAdvertisement()
		if !ok {
			t.Error("Unable to parse message as TSMPDiscoAdversitement")
		}
		if src.Addr() != pkgSrc {
			t.Errorf("Src address did not match, expected %v, got %v", pkgSrc, src.Addr())
		}
		keyData := key.Data[:]
		if !reflect.DeepEqual(keyData, discoKey) {
			t.Errorf("Key did not match, expected %q, got %q", discoKey, keyData)
		}
	})
}
