...
To run this example yourself:
 ...
 1. Add access controls which (i) define a new ACL tag, (ii) allow the demo
    node to host the Service, and (iii) allow peers on the tailnet to reach the
    Service. A sample ACL policy is provided below.

 2. Generate an auth key using the Tailscale admin panel. When doing so,
    add your new tag to your key (Service hosts must be tagged nodes).
