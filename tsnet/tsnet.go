...
func (s *Server) advertiseService(name tailcfg.ServiceName) error {
	...
	advertisedServices := s.lb.Prefs().AdvertiseServices()
	alreadyAdvertised := advertisedServices.ContainsFunc(func(s string) bool {
		return s == name.String()
	})
	if !alreadyAdvertised {
		newAdvertisedServices := make([]string, 0, advertisedServices.Len()+1)
		advertisedServices.AppendTo(newAdvertisedServices)
		_, err := s.lb.EditPrefs(&ipn.MaskedPrefs{
			AdvertiseServicesSet: true,
			Prefs: ipn.Prefs{
				AdvertiseServices: append(newAdvertisedServices, name.String()),
			},
		})
		if err != nil {
			return err
