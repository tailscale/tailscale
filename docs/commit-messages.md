...
But only do that if it’s actually a cleanup. Don’t use that as a shortcut to avoid filing a bug. Shortcuts to file bugs: http://go/bugc (corp, safe choice) or http://go/bugo (open source, if you want it public to the world).
...
- Moving away from a deprecated API to an equivalently behaving newer API
- Code movement
