...
func (nm *NetworkMap) TailnetDisplayName() string {
	if nm == nil || !nm.HasCap(tailcfg.NodeAttrTailnetDisplayName) {
		return ""
	}

	tailnetDisplayNames, err := tailcfg.UnmarshalNodeCapJSON[string](nm.SelfNode.CapMap().AsMap(), tailcfg.NodeAttrTailnetDisplayName)
