...
func TestParseArgs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "substring_filter_matches_indirect",
			args:        []string{"glob"},
...
func TestBranchName(t *testing.T) {
	...
	tests := []struct{ actor, want string }{
		{"bradfitz", "actions/bumpdep/bradfitz/20260917-220405"},
		{"some one/../evil", "actions/bumpdep/some-one-evil/20260917-220405"},
		{"", "actions/bumpdep/unknown/20260917-220405"},
	}
	for _, tt := range tests {
		if got := branchName(tt.actor, now); got != tt.want {
			t.Errorf("branchName(%q) = %q; want %q", tt.actor, got, tt.want)
		}
	}
}
...
func TestBuildReport(t *testing.T) {
	...
	env := &githubEnv{actor: "bradfitz", repo: "tailscale/tailscale"}
