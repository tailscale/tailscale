...
func buildReport(in reportInput) *report {
	...
	if in.env != nil {
		fmt.Fprintf(&body, "\nTriggered by @%s via the bumpdep workflow.\n", in.env.actor)
	}
	if in.issueRef != "" {
		fmt.Fprintf(&body, "\nUpdates %s\n", in.issueRef)
	}
	...
	if in.env != nil {
		r.branch = branchName(in.env.actor, in.now)
	}
	return r
}
...
// branchName returns the branch the workflow should push to. It's short
// and unique per run rather than descriptive: the old scheme of joining
// every module path produced names like
// actions/bumpdep/github.com-gokrazy-kernel.amd64-main-github.com-gokrazy-kernel.arm64-main-github.
func branchName(actor string, now time.Time) string {
	actor = strings.Trim(unsafeRefChars.ReplaceAllString(actor, "-"), "-")
	return fmt.Sprintf("actions/bumpdep/%s/%s", cmp.Or(actor, "unknown"), now.UTC().Format("20060102-150405"))
}
