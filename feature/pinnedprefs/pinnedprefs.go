...
// Package pinnedprefs stores the user's locally-pinned favorites (devices, exit nodes, and
// services) in a file per login profile. Pins are per-device and not synced across a user's devices.
package pinnedprefs
...
const featureName = "pinnedprefs"

// storeKey is the key under which a profile's pinned favorites are stored.
const storeKey = "pinned-prefs"
...
type extension struct {
	...
	// mu guards store and curPID, and the read/change/write in setPinnedPrefs.
	mu     sync.Mutex
