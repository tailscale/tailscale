...
func TestPinnedPrefsSetAndGet(t *testing.T) {
	e := newTestExtension(t, "pid")

	// Set devices and services in one request; exit-nodes untouched.
	got, err := e.setPinnedPrefs(ipn.PinnedPrefsRequest{
		Pinned: ipn.PinnedPrefs{
			Devices:  []ipn.PinnedItem{{ID: "n1"}, {ID: "n2"}},
			Services: []ipn.PinnedItem{{Service: "svc:web"}},
		},
		DevicesSet:  true,
		ServicesSet: true,
	})
	...
	got, err = e.setPinnedPrefs(ipn.PinnedPrefsRequest{
		Pinned:     ipn.PinnedPrefs{Devices: []ipn.PinnedItem{{ID: "n3"}}},
		DevicesSet: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	got, err = e.setPinnedPrefs(ipn.PinnedPrefsRequest{DevicesSet: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPinnedPrefsPersistAcrossStoreReopen(t *testing.T) {
	e := newTestExtension(t, "pid")
	if _, err := e.setPinnedPrefs(ipn.PinnedPrefsRequest{
		Pinned:     ipn.PinnedPrefs{Devices: []ipn.PinnedItem{{ID: "n1"}}},
		DevicesSet: true,
	}); err != nil {
		t.Fatal(err)
	}
...
func TestPinnedPrefsNoCurrentProfileReturnsEmpty(t *testing.T) {
	...
	if !got.Equals(ipn.PinnedPrefs{}) {
		t.Errorf("want zero PinnedPrefs, got %#v", got)
	}
}
