...
func TestHandlePacketsFromTunDevice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			...
			if want, got := tt.expectedFilterResponse, dph.HandlePacketsFromTunDevice(tt.p); want != got {
...
func TestHandlePacketsFromWireGuard(t *testing.T) {
	clientSrcIP := netip.MustParseAddr("100.70.0.1")
	...
	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			...
			if want, got := tt.expectedFilterResponse, dph.HandlePacketsFromWireGuard(tt.p); want != got {
				t.Errorf("unexpected filter response: want %v, got %v", want, got)
			}
			if want, got := tt.expectedSrc, tt.p.Src; want != got {
				t.Errorf("unexpected packet src: want %v, got %v", want, got)
...
func TestClientFlowCache(t *testing.T) {
	...
	o1 := outgoing
	if dph.HandlePacketsFromTunDevice(&o1) != filter.Accept {
		t.Errorf("first call to HandlePacketsFromTunDevice was not accepted")
	}
	...
	o2 := outgoing
	if dph.HandlePacketsFromTunDevice(&o2) != filter.Accept {
		t.Errorf("second call to HandlePacketsFromTunDevice was not accepted")
	}
	...
	if dph.HandlePacketsFromWireGuard(incoming) != filter.Accept {
		t.Errorf("call to HandlePacketsFromWireGuard was not accepted")
	}
...
func TestConnectorFlowCache(t *testing.T) {
	...
	o1 := outgoing
	if dph.HandlePacketsFromWireGuard(&o1) != filter.Accept {
		t.Errorf("first call to HandlePacketsFromWireGuard was not accepted")
	}
	...
	// The second call should use the cache.
	o2 := outgoing
	if dph.HandlePacketsFromWireGuard(&o2) != filter.Accept {
		t.Errorf("second call to HandlePacketsFromWireGuard was not accepted")
	}
	...
	if dph.HandlePacketsFromTunDevice(incoming) != filter.Accept {
		t.Errorf("call to HandlePacketsFromTunDevice was not accepted")
	}
