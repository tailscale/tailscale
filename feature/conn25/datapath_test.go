...
type testNet struct {
	name        string
	ipVersion   uint8
	ipHeaderLen int // bytes of IP header preceding the transport header
...
func (tn testNet) synthParsed(src, dst netip.AddrPort) *packet.Parsed {
	p := &packet.Parsed{
		IPProto:   ipproto.UDP,
		IPVersion: tn.ipVersion,
		Src:       src,
		Dst:       dst,
	}
	p.StuffForTesting(tn.ipHeaderLen + 8)
	return p
}
		...
		t.Run(tn.name, func(t *testing.T) {
			for _, tt := range tests {
				t.Run(tt.description, func(t *testing.T) {
					...
					p := tn.synthParsed(tt.src, tt.dst)
