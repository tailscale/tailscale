...
func TestReturnAddrs(t *testing.T) {
	c := newConn25(t.Logf)
	...
	t.Run("return-ok-v4", func(t *testing.T) {
		magic, err := c.client.v4MagicIPPool.next()
		if err != nil {
			t.Errorf("get v4 magic ip: %v", err)
		}
		transit, err := c.client.v4TransitIPPool.next()
		if err != nil {
			t.Errorf("get v4 transit ip: %v", err)
		}
		addrs := &addrs{
			magic:   magic,
			transit: transit,
			dst:     netip.MustParseAddr("192.0.2.42"),
			domain:  dnsname.FQDN("utilized-and-returnable.example."),
		}
		if err := c.client.assignments.insert(addrs); err != nil {
			t.Errorf("inserting assignment: %v", err)
		}
		if err := c.client.returnAddrs(addrs); err != nil {
			t.Errorf("returnAddr: expected no error, got: %v", err)
		}
		if c.client.v4MagicIPPool.inUse.Contains(magic) {
			t.Errorf("found magic ip that should have been returned: %s", magic)
		}
		if c.client.v4TransitIPPool.inUse.Contains(transit) {
			t.Errorf("found transit ip that should have been returned: %s", transit)
		}
	})

	t.Run("return-not-in-use-v4", func(t *testing.T) {
		// In the range but not marked inUse by next().
		magic := netip.MustParseAddr("100.64.0.1")
		transit := netip.MustParseAddr("169.254.0.1")
		addrs := &addrs{
			magic:   magic,
			transit: transit,
			dst:     netip.MustParseAddr("192.0.2.42"),
			domain:  dnsname.FQDN("wild-undomesticated-addr.example."),
		}
		if err := c.client.assignments.insert(addrs); err != nil {
			t.Errorf("inserting assignment: %v", err)
		}
		if err := c.client.returnAddrs(addrs); !errors.Is(err, errAddrExists) {
			t.Errorf("returnAddr: expected error %v, got none", errAddrExists)
		}
	})

	t.Run("return-ok-v6", func(t *testing.T) {
		magic, err := c.client.v6MagicIPPool.next()
		if err != nil {
			t.Errorf("get v6 magic ip: %v", err)
		}
		transit, err := c.client.v6TransitIPPool.next()
		if err != nil {
			t.Errorf("get v6 transit ip: %v", err)
		}
		addrs := &addrs{
			magic:   magic,
			transit: transit,
			dst:     netip.MustParseAddr("3fff::1"),
			domain:  dnsname.FQDN("utilized-and-returnable.example."),
		}
		if err := c.client.assignments.insert(addrs); err != nil {
			t.Errorf("inserting assignment: %v", err)
		...
		if c.client.v6MagicIPPool.inUse.Contains(magic) {
			t.Errorf("found magic ip that should have been returned: %s", magic)
		}
		if c.client.v6TransitIPPool.inUse.Contains(transit) {
			t.Errorf("found transit ip that should have been returned: %s", transit)
		}
	})

	t.Run("return-not-in-use-v6", func(t *testing.T) {
		// In the range but not marked inUse by next().
		magic := netip.MustParseAddr("fd7a:115c:a1e0:a99c:0100::1")
		transit := netip.MustParseAddr("fd7a:115c:a1e0:a99c:0200::1")
		addrs := &addrs{
			magic:   magic,
			transit: transit,
			dst:     netip.MustParseAddr("3fff::1"),
