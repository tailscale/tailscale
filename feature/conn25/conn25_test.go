...
func parseARecords(t *testing.T, buf []byte) []netip.Addr {
	t.Helper()
	var result []netip.Addr
	for _, r := range parseAnswers(t, buf) {
		if b, ok := r.Body.(*dnsmessage.AResource); ok {
			result = append(result, netip.AddrFrom4(b.A))
		}
	}
	return result
}
...
func TestMapDNSResponseRewritesResponses(t *testing.T) {
	sn := makeSelfNode(t, appctype.Conn25Attr{
		...
		Domains:    []string{"example.com"},
		MagicIPPool: []netipx.IPRange{
			rangeFrom("0", "10"),
			netipx.IPRangeFrom(
				netip.MustParseAddr("fd7a:115c:a1e0::"),
				netip.MustParseAddr("fd7a:115c:a1e0::10"),
			),
		},
		TransitIPPool: []netipx.IPRange{
			rangeFrom("40", "50"),
			netipx.IPRangeFrom(
				netip.MustParseAddr("fe80:7a11:5ca1:e077::"),
				netip.MustParseAddr("fe80:7a11:5ca1:e077::10"),
			),
		},
	}, []string{})
	...
	for _, tt := range []struct {
		...
		{
			name: "a-records-rewritten-to-magic-ips",
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			result := c.mapDNSResponse(tt.dnsResp)

			if diff := cmp.Diff(tt.wantA, parseARecords(t, result), cmpopts.EquateComparable(netip.Addr{})); diff != "" {
