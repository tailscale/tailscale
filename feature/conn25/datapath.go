...
type IPMapper interface {
	...
	// If the magicIP within a configured Magic IP range for an app on the client,
...
func newDatapathHandler(ipMapper IPMapper, logf logger.Logf) *datapathHandler {
	return &datapathHandler{
		...
		clientFlowTable:    NewFlowTable(100),
		connectorFlowTable: NewFlowTable(100),
		logf:               logf,
		debugLogging:       envknob.Bool("TS_CONN25_DATAPATH_DEBUG"),
	}
}

// HandlePacketsFromWireGuard inspects packets coming from WireGuard, and performs
...
func (dh *datapathHandler) HandlePacketsFromWireGuard(p *packet.Parsed) filter.Response {
	...
	if err != nil && !errors.Is(err, ErrFlowNotFound) {
		dh.debugLogf("error reading client flow table, passing packet unmodified: %v", err)
	...
	if err != nil && !errors.Is(err, ErrFlowNotFound) {
		dh.debugLogf("error reading connector flow table, passing packet unmodified: %v", err)
		return filter.Accept
	}

	// Perform the action for the existing connector outbound flow and return.
	if err == nil {
		existing.Action(p)
		return filter.Accept
	}
	...
	// or other (non-app-connector) traffic.
	...
	if err := dh.connectorFlowTable.NewFlowFromWireGuard(
		outgoing,
		FlowData{
...
// HandlePacketsFromTunDevice inspects packets coming from the tun device, and performs
// appropriate DNAT or SNAT actions for Connectors 2025. Returning [filter.Accept] signals
// that the packet should pass through subsequent stages of the datapath pipeline.
// Returning [filter.Drop] signals the packet should be dropped. This method handles all
...
func (dh *datapathHandler) HandlePacketsFromTunDevice(p *packet.Parsed) filter.Response {
	...
	// Perform the action for the existing client outbound flow and return.
	if err == nil {
		existing.Action(p)
		return filter.Accept
	}
	...
	// Perform the action for the existing connector return flow and return.
	if err == nil {
		existing.Action(p)
		return filter.Accept
	}
	...
	if err := dh.clientFlowTable.NewFlowFromTunDevice(
		outgoing,
		FlowData{
