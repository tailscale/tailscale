	...
	if exists {
		...
		case webrtc.SignalingStateStable:
			if state == stateConnected || state == stateConnecting {
				...
				// and stop retrying.
				m.logf("webrtc: ignoring offer from %v, already have %v connection", remoteDisco.ShortString(), state)
				return
			}
			...
			m.logf("webrtc: tearing down stale %v connection to %v, answering fresh offer", state, remoteDisco.ShortString())
			ps.peerConn.Close()
			m.mu.Lock()
			delete(m.peerConnectionsByPeer, ps.peer)
			delete(m.peerConnectionsByDisco, remoteDisco)
