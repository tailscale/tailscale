...
func TestResolveAuthKey(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			idToken:  "",
			tags:     []string{"tag:test"},
			wantErr:  "identity federation authkeys require --wif-id-token",
		},
		{
			name:     "missing tags",
			clientID: "client-123",
...
func mockedControlServer(t *testing.T) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if bytes.Contains([]byte(r.URL.Path), []byte("/oauth/token-exchange")) {
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"access_token":"access-123","token_type":"Bearer","expires_in":3600}`))
			return
		}
		if bytes.Contains([]byte(r.URL.Path), []byte("/api/v2/tailnet")) && bytes.Contains([]byte(r.URL.Path), []byte("/keys")) {
			w.WriteHeader(http.StatusOK)
