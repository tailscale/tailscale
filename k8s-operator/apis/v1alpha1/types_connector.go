...
// +kubebuilder:validation:XValidation:rule="!((has(self.subnetRouter) || (has(self.exitNode)  && self.exitNode == true)) && has(self.appConnector))",message="A Connector must not be both app connector and either of exit node or subnet router."
type ConnectorSpec struct {
	...
	// AppConnector defines whether the Connector node should act as a Tailscale app connector. A Connector that is
	...
	// Note that the Connector only creates a Tailscale device that acts as an app connector. You will need to
	// configure the permissions and the domains separately via the Admin panel.
	// It is also your responsibility to enforce a predictable egress IP for the connector, for example by enforcing
	// that cluster egress traffic is routed via egress NAT with a static public IP address.
	...
	// ExitNode defines whether the Connector node should act as a Tailscale exit node. Defaults to false.
