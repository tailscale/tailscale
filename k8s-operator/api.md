...
| `useLetsEncryptStagingEndpoint` _boolean_ | UseLetsEncryptStagingEndpoint can be set to true to use the<br />LetsEncrypt staging endpoint for the TLS certs issued for any HTTPS<br />endpoints exposed to tailnet by a proxy instance.<br />In practice, this only applies to Tailscale Ingress tailnet endpoint.<br />https://letsencrypt.org/docs/staging-environment/<br />By default these certs are issued by the LetsEncrypt production<br />endpoint.<br />Note, that changing this setting true -> false, will result in any<br />existing certs being re-issued from the production endpoint, however<br />if you have already provisioned certs from the production endpoint<br />and set this to true, the existing production certs might be used till the time<br />comes to renew them. |  |  |
