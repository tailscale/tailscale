...
func allocatePorts(ctx context.Context, c client.Client, cfg Config) (map[string]uint16, error) {
	...
	if replicasAllocated < int(cfg.Replicas) {
		return nil, &AllocatePortsError{msg: fmt.Sprintf("not enough available ports to allocate all replicas (needed %d, got %d). Field 'spec.staticEndpoints.nodePort.ports' on ProxyClass %q must have bigger range allocated", cfg.Replicas, replicasAllocated, cfg.ProxyClassName)}
	}
