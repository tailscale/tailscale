...
const (
	...
	dnsRecordsReconcilerFinalizer = "tailscale.com/dns-records-reconciler"
...
func (r *Reconciler) maybeProvision(ctx context.Context, proxySvc *corev1.Service, logger *zap.SugaredLogger) error {
	...
	// Stamp the finalizer to help with records cleanup when proxy resources are deleted. The change is persisted
	// alongside the annotation update below in a single r.Update call.
	reconciler.SetFinalizer(proxySvc, dnsRecordsReconcilerFinalizer)
...
func (r *Reconciler) maybeCleanup(ctx context.Context, proxySvc *corev1.Service, logger *zap.SugaredLogger) error {
	if !slices.Contains(proxySvc.Finalizers, dnsRecordsReconcilerFinalizer) {
...
func (r *Reconciler) removeProxySvcFinalizer(ctx context.Context, proxySvc *corev1.Service) error {
	return reconciler.ClearFinalizer(ctx, r.Client, proxySvc, dnsRecordsReconcilerFinalizer)
}
...
func (r *Reconciler) isSvcForFQDNEgressProxy(ctx context.Context, svc *corev1.Service) (bool, error) {
	if !reconciler.IsManagedByType(svc, serviceTypeSvc) {
		return false, nil
	}
...
func endpointSliceHandler(ctx context.Context, o client.Object) []reconcile.Request {
	if !reconciler.IsManagedByType(o, serviceTypeSvc) && !reconciler.IsManagedByType(o, serviceTypeIngress) {
		return nil
	}
...
func ingressHandlerForNamespace(ns string, isDefaultLoadBalancer bool, cl client.Client, logger *zap.SugaredLogger) handler.MapFunc {
	return func(ctx context.Context, o client.Object) []reconcile.Request {
		...
		proxyResourceLabels := reconciler.Labels(serviceTypeIngress, ing.Name, ing.Namespace)
		headlessSvc, err := reconciler.GetSingleObject[corev1.Service](ctx, cl, ns, proxyResourceLabels)
		if err != nil {
...
func enqueueAllIngressEgressProxySvcsInNS(ns string, cl client.Client, logger *zap.SugaredLogger) handler.MapFunc {
	return func(ctx context.Context, _ client.Object) []reconcile.Request {
		...
		if err := cl.List(ctx, svcHeadlessSvcList, client.InNamespace(ns), client.MatchingLabels(reconciler.SelectorForType(serviceTypeSvc))); err != nil {
		...
		if err := cl.List(ctx, ingHeadlessSvcList, client.InNamespace(ns), client.MatchingLabels(reconciler.SelectorForType(serviceTypeIngress))); err != nil {
