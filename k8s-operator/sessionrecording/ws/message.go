...
func fragmentDimensions(b []byte, maskSet bool) (payloadLength, payloadOffset, maskOffset uint64, _ error) {
	...
	// Sanity check- ensure that a potentially incorrectly received huge
	// payload length doesn't break us down the line when we attempt to create
	// a new byte slice for its contents.
	if rawLength := payloadLength + payloadOffset; rawLength > math.MaxInt64 {
		return 0, 0, 0, fmt.Errorf("[unexpected]: too large expected raw message size: %v", rawLength)
	}
