...
// Test_spdyFrame_parseHeaders_decompressionBomb verifies that a control frame
// whose zlib-compressed Name/Value block decompresses to more than
// maxHeaderBlockLength is rejected rather than expanded into memory. A tiny
// frame can otherwise declare a multi-GB value that zlib inflates ~1000x.
func Test_spdyFrame_parseHeaders_decompressionBomb(t *testing.T) {
	zl, err := zap.NewDevelopment()
	if err != nil {
		t.Fatal(err)
	}

	// buildBlock builds a SYN_STREAM payload with a single header whose value
	// is a valueLen-byte run of one byte (compresses tiny, decompresses large).
	buildBlock := func(valueLen int) []byte {
		buf := bytes.NewBuffer(nil)
		writeControlFramePayloadBeforeHeaders(t, buf, SYN_STREAM, 1)
		...
		if err := binary.Write(w, binary.BigEndian, uint32(valueLen)); err != nil {
			t.Fatal(err)
		}
		if _, err := w.Write(bytes.Repeat([]byte("A"), valueLen)); err != nil {
			t.Fatal(err)
		}
		w.Flush()
		w.Close()
	...
	oversized := buildBlock(maxHeaderBlockLength + 1)
	var z zlibReader
	sf := &spdyFrame{Ctrl: true, Type: SYN_STREAM, Payload: oversized}
	if _, err := sf.parseHeaders(&z, zl.Sugar()); err == nil {
		t.Fatalf("parseHeaders accepted a header block that decompresses past the %d byte cap", maxHeaderBlockLength)
	}

	// A block that stays under the cap must still parse.
	small := buildBlock(16)
	var z2 zlibReader
	sf2 := &spdyFrame{Ctrl: true, Type: SYN_STREAM, Payload: small}
	got, err := sf2.parseHeaders(&z2, zl.Sugar())
	if err != nil {
		t.Fatalf("parseHeaders rejected a valid header block: %v", err)
	}
