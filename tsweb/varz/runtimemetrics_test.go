...
func TestRuntimeMetricSpecs(t *testing.T) {
	...
	for _, spec := range runtimeMetricSpecs {
		...
		if spec.leIsLowerBound && d.Kind != metrics.KindFloat64Histogram {
			t.Errorf("spec %q sets leIsLowerBound but is not a histogram", spec.name)
		}
	}
...
func TestRuntimeHistLayout(t *testing.T) {
	t.Run("time", func(t *testing.T) {
		...
		lay := newRuntimeHistLayout("m", buckets, false)
		want := []string{
	...
	t.Run("lower_bound", func(t *testing.T) {
		...
		lay := newRuntimeHistLayout("m", buckets, true)
