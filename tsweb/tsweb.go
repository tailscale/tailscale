...
func (h handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	lw := loggingResponseWriter{w, 0, h.logf}
	var err error
	if _, ok := w.(http.Flusher); ok {
		err = h.h.ServeHTTPErr(loggingResponseWriterFlusher{&lw}, r)
	} else {
		err = h.h.ServeHTTPErr(&lw, r)
	}
	hErr, hErrOK := err.(HTTPError)

	msg.HTTP.Code = lw.code
	msg.Duration = h.timeNow().Sub(msg.When)

	switch {
	case err != nil && r.Context().Err() == context.Canceled:
		// TODO(bradfitz): this 499 thing is weird and undocumented.
	...
	case hErrOK:
		...
		if msg.HTTP.Code == 0 {
			...
			msg.HTTP.Code = http.StatusInternalServerError
		}
		http.Error(&lw, msg.Msg, msg.HTTP.Code)
	case err != nil:
	...
	case lw.code == 0:
		...
		msg.HTTP.Code = http.StatusInternalServerError
		msg.Err = errors.New("[unexpected] handler did not respond to the client")
		msg.Msg = "internal server error"
		http.Error(&lw, msg.Msg, msg.HTTP.Code)
	}
...
func (l *loggingResponseWriter) Write(bs []byte) (int, error) {
	...
	return l.ResponseWriter.Write(bs)
}
...
// loggingResponseWriterFlusher augments loggingResponseWriter with
// and http.Flusher implementation. Callers must only use this struct
// if the loggingResponseWriter's underlying ResponseWriter implements
// Flush.
type loggingResponseWriterFlusher struct {
	*loggingResponseWriter
}

func (l loggingResponseWriterFlusher) Flush() {
	f, _ := l.loggingResponseWriter.ResponseWriter.(http.Flusher)
	if f == nil {
		l.loggingResponseWriter.logf("[unexpected] loggingResponseWriterFlusher was given an http.ResponseWriter that doesn't implement http.Flusher, Flush() call is a a no-op")
		return
	}
	f.Flush()
}
