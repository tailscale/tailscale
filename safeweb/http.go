...
func (s *Server) serveBrowser(w http.ResponseWriter, r *http.Request) {
	...
	if !slices.Contains([]string{"GET", "HEAD", "OPTIONS"}, r.Method) && r.Header.Get("Sec-Fetch-Site") != "same-origin" {
		// deny non-GET/HEAD/OPTIONS requests that are not same-origin
