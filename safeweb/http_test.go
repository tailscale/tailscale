...
func TestCSRFProtection(t *testing.T) {
	tests := []struct {
		...
		httpMethod      string
		apiRoute        bool
		setSecFetchSite bool
		wantStatus      int
	}{
		{
			name:            "GET requests to non-API routes do not require Sec-Fetch-Site header",
			httpMethod:      http.MethodGet,
			apiRoute:        false,
			setSecFetchSite: false,
			wantStatus:      http.StatusOK,
		},
		{
			name:            "POST requests to non-API routes require Sec-Fetch-Site=same-origin and fail if not provided",
		...
		{
			name:            "POST requests to non-API routes require Sec-Fetch-Site=same-origin and pass if provided",
			httpMethod:      http.MethodPost,
			apiRoute:        false,
			setSecFetchSite: true,
			wantStatus:      http.StatusOK,
		},
		{
