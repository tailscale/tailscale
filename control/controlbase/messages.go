...
const (
	...
	pqInitiationPayloadLen = 32 + (mlkem.EncapsulationKeySize768 + 16) + 48 + 16
	pqResponsePayloadLen   = 32 + (mlkem.CiphertextSize768 + 16) + 16
)
...
// client machine to a control server. It carries the "-> e, es, e1, s, ss"
// message of the Noise IKhfs pattern; see [pqProtocolName].
//
// 2b: protocol version
// 1b: message type (0x01)
// 2b: payload length (1296)
// 32b: client ephemeral public key (cleartext)
// 1200b: client ML-KEM-768 encapsulation key (encrypted)
// 48b: client machine public key (encrypted)
// 16b: message tag (authenticates the whole message)
...
func (m *pqInitiationMessage) MLKEMEncapsulationKey() []byte {
	return m[initiationHeaderLen+32 : initiationHeaderLen+32+mlkem.EncapsulationKeySize768+16]
}
func (m *pqInitiationMessage) MachinePub() []byte {
	return m[initiationHeaderLen+32+mlkem.EncapsulationKeySize768+16 : initiationHeaderLen+32+mlkem.EncapsulationKeySize768+16+48]
}
func (m *pqInitiationMessage) Tag() []byte {
	return m[initiationHeaderLen+32+mlkem.EncapsulationKeySize768+16+48:]
}
...
// control server to a client machine. It carries the "<- e, ee, ekem1, se"
// message of the Noise IKhfs pattern; see [pqProtocolName].
//
// 1b: message type (0x02)
// 2b: payload length (1152)
// 32b: control ephemeral public key (cleartext)
// 1104b: ML-KEM-768 ciphertext (encrypted)
// 16b: message tag (authenticates the whole message)
...
func (m *pqResponseMessage) MLKEMCiphertext() []byte {
	return m[headerLen+32 : headerLen+32+mlkem.CiphertextSize768+16]
}
func (m *pqResponseMessage) Tag() []byte { return m[headerLen+32+mlkem.CiphertextSize768+16:] }
