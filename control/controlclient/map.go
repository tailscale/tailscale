...
const (
	sourceControl updateSource = iota
	sourceTSMP
)
...
func (ms *mapSession) run() {
	...
	for {
		...
		case change := <-ms.changeQueue:
			ms.handleNonKeepAliveMapResponse(ms.sessionAliveCtx, change.response, change.source)
...
func (ms *mapSession) handleNonKeepAliveMapResponse(ctx context.Context,
	resp *tailcfg.MapResponse, source updateSource,
) error {
...
func (ms *mapSession) tryMarkDiscoAsLearnedFromTSMP(res *tailcfg.MapResponse) {
	dun, ok := ms.netmapUpdater.(DiscoUpdateNotifier)
	if !ok {
		return
	}
	...
	for _, change := range res.PeersChangedPatch {
		...
		dun.MarkDiscoAsLearnedFromTSMP(*change.Key, *change.DiscoKey)
	}
}
