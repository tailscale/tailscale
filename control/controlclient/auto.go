...
func (c *Auto) computeStateLocked() State {
	if c.inMapPoll && c.loggedIn {
		return StateSynchronized
	}
	if c.wantLoggedIn && c.urlToVisit == "" && !c.loggedIn {
		return StateAuthenticating
	}
	if c.wantLoggedIn && c.urlToVisit != "" && !c.loggedIn {
		return StateURLVisitRequired
	}
	if c.wantLoggedIn && c.loggedIn && !c.inMapPoll {
...
func (c *Auto) sendStatus(who string, err error, url string, nm *netmap.NetworkMap) {
	...
	state := c.state
	state2 := c.computeStateLocked()
	loggedIn := c.loggedIn
	inMapPoll := c.inMapPoll
	c.inSendStatus++

	if false && state != state2 {
		panic(fmt.Sprintf("state %v does not equal computed %v (loggedIn=%v, wantLoggedIn=%v, inMapPoll=%v, urlToVisit=%q)",
			state, state2,
			c.loggedIn,
			c.wantLoggedIn,
			c.inMapPoll,
