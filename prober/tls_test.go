...
func TestCRL(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			cs := &tls.ConnectionState{PeerCertificates: []*x509.Certificate{leafCert, caCert}}
			if tt.crlBytes != nil {
				crlServer.crlBytes = tt.crlBytes
				leafCert.CRLDistributionPoints = []string{srv.URL}
			} else {
				crlServer.crlBytes = nil
				leafCert.CRLDistributionPoints = []string{}
			}
			err := validateConnState(context.Background(), cs)
