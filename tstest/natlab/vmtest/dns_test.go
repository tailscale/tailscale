...
//
// vnet's second DNS server refuses [vnet.SplitDNSRefusedName], serves no DNS
// over TCP, and drops rather than refuses TCP to its port. A forwarder that
// retries the rcode over TCP therefore waits on an answer that never comes.
// Before this fix, such a query burned the full 10s dnsQueryTimeout and then
// returned no response at all.
//
// Lookups go through getent, which is libc over UDP, and that is the path a
// real client takes. "tailscale dns query" would be easier to assert on,
// because it reports the rcode directly. LocalAPI queries are tcp-family
// though, and the forwarder races UDP and TCP from the start for those by
// design, so they exercise a different path. The direct DNS backend keeps
// systemd-resolved from retrying the REFUSED over TCP itself. Such a retry
// would bump the same TCP counter the test asserts on.
//
// The assertion is on tailscaled's counters rather than on wall-clock time. An
// emulated guest takes seconds just to start a process, and the retry's cost
// depends on how the upstream refuses TCP.
//
...
func TestForwardedRefusedRcode(t *testing.T) {
	...
	// A name the same server answers normally, as a positive control. Retrying
	// it also waits out the asynchronous application of the DNS config.
	// Without this, the REFUSED below could just mean the route never landed.
	if err := tstest.WaitFor(30*time.Second, func() error {
