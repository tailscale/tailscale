...
// TestSplitDNSEmptyBaseConfig checks that tailscaled leaves DNS alone when the
// OS has no upstream resolvers, and takes over once they appear. Under the
// "direct" backend the "." route comes only from /etc/resolv.conf, so emptying
// that file and reconfiguring stands in for a boot that beats
// NetworkManager/systemd-resolved.
//
// The node joins with a working resolver first: tailscaled finds the control
// plane by DNS name, so a node with no resolver never gets far enough to
// configure DNS.
// See https://github.com/tailscale/tailscale/issues/20341
func TestSplitDNSEmptyBaseConfig(t *testing.T) {
	const (
		fwdName = vnet.SplitDNSName
		fwdIP   = vnet.SplitDNSAddr
	)
	...
	env := vmtest.New(t,
		vmtest.ControlDNS("tailnet.test", &tailcfg.DNSConfig{
			// MagicDNS plus a routed domain, and no global resolvers, so the
			// "." route comes from the OS base config.
			Proxied: true,
			Routes: map[string][]*dnstype.Resolver{
				vnet.SplitDNSDomain: {{Addr: vnet.FakeSplitDNSIPv4().String()}},
			},
		}))
	...
	env.Start()
	env.AssertDNSBackend(client, "direct")

	// Empty resolv.conf while tailscaled isn't managing it, so the next config
	// pass reads a base config with no upstream resolvers. The backup goes too,
	// else tailscaled recovers the old resolvers from it.
	env.SetAcceptDNS(client, false)
	const emptyResolvConf = "rm -f /etc/resolv.conf /etc/resolv.pre-tailscale-backup.conf && : >/etc/resolv.conf"
	if out, err := env.SSHExec(client, emptyResolvConf); err != nil {
		t.Fatalf("emptying resolv.conf: %v (%s)", err, strings.TrimSpace(out))
	}
	env.SetAcceptDNS(client, true)

	// quad-100 must not become the OS resolver with no upstream to forward to.
	// Public names don't resolve either way here, since the guest has none, so
	// this is the assertion that catches the bug.
	assertResolverState(t, env, client, nil, []string{"100.100.100.100"})
	out, err := env.SSHExec(client, "cat /etc/resolv.conf")
	if err != nil {
		t.Fatalf("cat /etc/resolv.conf: %v (%s)", err, strings.TrimSpace(out))
	}
	if strings.Contains(out, "generated by tailscale") {
		t.Errorf("/etc/resolv.conf is tailscale-generated, want it untouched:\n%s", out)
	}

	// The OS publishes a resolver. vnet's fake DNS server is the only one
	// reachable here, and it serves the public names below.
	populate := fmt.Sprintf("printf 'nameserver %s\\n' >/etc/resolv.conf", vnet.FakeDNSIPv4())
	if out, err := env.SSHExec(client, populate); err != nil {
		t.Fatalf("populating resolv.conf: %v (%s)", err, strings.TrimSpace(out))
	}

	// tailscaled's retry should notice and take over.
	assertResolverState(t, env, client, []string{"100.100.100.100"}, nil)

	// Both halves of the config it just claimed work: the routed domain, and
	// public names via the upstream it picked up.
	checks := []struct {
		name string // what to look up on the client
		want string // the answer proving the right resolver handled it
		what string // for failure messages
		...
		},
		{
			name: "dualstack-web.example.com",
			want: "5.0.0.100",
			what: "default: public name resolved via the newly-published upstream",
		},
	}
	for _, c := range checks {
		if err := tstest.WaitFor(60*time.Second, func() error {
			...
			out, _ := env.SSHExec(client, "cat /etc/resolv.conf")
			t.Fatalf("%v\nclient resolv.conf:\n%s", err, out)
		}
	}
}
