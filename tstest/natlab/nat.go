...
type mapping struct {
	// both tuples are from the POV of outbound packets, i.e. src is
	// always the "local" side of the session.
	lanSrc   netaddr.IPPort
	lanDst   netaddr.IPPort
	wanSrc   netaddr.IPPort
	deadline time.Time
	pc       net.PacketConn // parks the allocated wan port
}
...
// NAT44 implements an IPv4-to-IPv4 source NAT (SNAT) translator, with
...
func (n *NAT44) timeNow() time.Time {
	if n.TimeNow != nil {
		return n.TimeNow()
	}
	return time.Now()
...
func (n *NAT44) HandlePacket(p *Packet, inIf *Interface) PacketVerdict {
	n.mu.Lock()
	defer n.mu.Unlock()
	n.initLocked()
...
func (n *NAT44) processInboundLocked(p *Packet, inIf *Interface) PacketVerdict {
...
func (n *NAT44) processOutboundLocked(p *Packet, inIf *Interface) PacketVerdict {
	if n.Firewall != nil {
...
func (n *NAT44) allocateMappedPort() (net.PacketConn, netaddr.IPPort) {
