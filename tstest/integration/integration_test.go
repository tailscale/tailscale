...
func authURLHandler(t *testing.T, control *testcontrol.Server) *authURLParserWriter {
	t.Helper()
		...
		for _, useSeamlessKeyRenewal := range []bool{true, false} {
			t.Run(fmt.Sprintf("%s-seamless-%t", tt.name, useSeamlessKeyRenewal), func(t *testing.T) {
				...
				if tt.alreadyLoggedIn {
					...
					cmd.Stdout = authURLHandler(t, env.Control)
				...
				cmd := n1.Tailscale(cmdArgs...)
				handler := authURLHandler(t, env.Control)
				cmd.Stdout = handler
				cmd.Stderr = cmd.Stdout
				...
				if n := handler.authURLCount.Load(); n != expectedAuthUrls {
...
func TestOneNodeUpInterruptedAuth(t *testing.T) {
	...
	cmd2.Stdout = authURLHandler(t, env.Control)
