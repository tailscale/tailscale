...
var (
	...
	hostname    = flag.String("hostname", "derp.tailscale.com", "TLS host name for certs, if addr's port is :443. It can be one IP address when --certmode=manual (to avoid SNI checks). With --acme-ip-certs, it can instead be one IP address or a comma-separated IPv4 and IPv6 pair to run an IP-only server; each configured address overrides the connection-local address of the same family for servers behind NAT.")
	...
	acmeIPCerts = flag.Bool("acme-ip-certs", false, "whether to serve LetsEncrypt certs for the server's IP addresses: when a client connects by IP address (sending no TLS SNI, or an IP address SNI matching the server IP selected for the connection), get and serve a LetsEncrypt cert for that IP, using the short-lived (~6 day) ACME certificate profile. This works for both IPv4 and IPv6 with no per-address configuration. For servers behind destination NAT, an IP-valued --hostname can override the post-NAT destination address. It requires --certmode=letsencrypt and the ACME server must be able to reach port 80 at each such IP for the HTTP-01 challenge.")
