...
func main() {
	...
	if *runSTUN {
		stunListenHost, stunListenPort, err := net.SplitHostPort(*stunPort)
		if err != nil {
			stunListenHost = listenHost
			stunListenPort = *stunPort
		}
		ss := stunserver.New(ctx)
		go ss.ListenAndServe(net.JoinHostPort(stunListenHost, stunListenPort))
	}
