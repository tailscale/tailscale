...
func (r *ProxyGroupReconciler) maybeExposeViaNodePort(ctx context.Context, pc *tsapi.ProxyClass, pg *tsapi.ProxyGroup, logger *zap.SugaredLogger) (map[string]int32, bool, error) {
	...
	for i := range pgReplicas(pg) {
		...
		if port == 0 {
			...
			logger.Infof("ProxyGroup %q replica %q exposed on NodePort %d. Please ensure the appropriate firewall rules are configured to expose it on the desired network.", pg.Name, svc.Name, port)
		}
	}
...
func (r *ProxyGroupReconciler) ensureConfigSecretsCreated(ctx context.Context, pg *tsapi.ProxyGroup, proxyClass *tsapi.ProxyClass, ports map[string]int32) (hash string, err error) {
	...
	for i := range pgReplicas(pg) {
		...
		if len(ports) > 0 {
			...
			endpoints, err = r.findStaticEndpoints(ctx, port, proxyClass, logger)
			if err != nil {
		...
		configs, err := pgTailscaledConfig(pg, proxyClass, i, authKey, existingCfgSecret, endpoints, logger)
		if err != nil {
...
func (r *ProxyGroupReconciler) findStaticEndpoints(ctx context.Context, port int32, proxyClass *tsapi.ProxyClass, logger *zap.SugaredLogger) ([]netip.AddrPort, error) {
	...
	if len(selectors) < 1 {
		return nil, fmt.Errorf("no LabelSelectors specified on tailnetListenerConfig for ProxyClass %q", proxyClass.Name)
	}
