...
func setupTest() (client.Client, *zap.SugaredLogger) {
	cl := fake.NewClientBuilder().WithScheme(tsapi.GlobalScheme).Build()
...
func TestValidateNodePortRanges(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			portRanges: []tsapi.PortRange{
				{Port: 30000, EndPort: 30009},
				{Port: 30010, EndPort: 30020},
			},
			wantErr: false,
		},
		{
			name:       "empty_port_ranges",
			portRanges: []tsapi.PortRange{},
			wantErr:    false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := validateNodePortRanges(context.Background(), cl, "test-ns", tt.portRanges, logger)
			if (err != nil) != tt.wantErr {
