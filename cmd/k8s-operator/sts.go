...
func (a *tailscaleSTSReconciler) createOrGetSecrets(ctx context.Context, logger *zap.SugaredLogger, stsC *tailscaleSTSConfig, hsvc *corev1.Service) ([]string, error) {
	...
	for i := range stsC.Replicas {
		...
		// If we only have a single replica, use the hostname verbatim. Otherwise, add an ordinal suffix
		// of the replica number to the hostname.
		hostname := stsC.Hostname
		if stsC.Replicas > 1 {
			hostname = hostname + "-" + strconv.FormatInt(int64(i), 10)
		}
