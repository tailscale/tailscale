...
func (a *tailscaleSTSReconciler) provisionSecrets(ctx context.Context, logger *zap.SugaredLogger, stsC *tailscaleSTSConfig, hsvc *corev1.Service) ([]string, error) {
	...
	// To ensure we only remove the extra secrets, we'll order them by their ordinal suffix in ascending order. That way,
	// when we go from 5 to 3, we'd be able to easily remove the secrets/devices for 4 and 5.
	slices.SortFunc(secrets.Items, func(a, b corev1.Secret) int {
		ai := strings.LastIndex(a.Name, "-")
		bi := strings.LastIndex(b.Name, "-")
		an, _ := strconv.Atoi(a.Name[ai+1:])
		bn, _ := strconv.Atoi(b.Name[bi+1:])
		return an - bn
	})

	if len(secrets.Items) > int(stsC.Replicas) {
		for _, secret := range secrets.Items[stsC.Replicas:] {
