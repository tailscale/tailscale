...
type tailscaleSTSConfig struct {
	...
	// CustomTLSHost is the SNI hostname for a custom TLS certificate (single-Ingress mode).
	CustomTLSHost string
	// CustomTLSData contains the raw TLS secret data (tls.crt, tls.key) to mount into the proxy (single-Ingress mode).
	CustomTLSData map[string][]byte

	// CustomTLSCerts is used in shared-proxy mode. It maps hostname to TLS secret data
	// (each value is a map with keys like "tls.crt" and "tls.key").
	CustomTLSCerts map[string]map[string][]byte
...
// syncTLSSecret creates (or updates) a copy of the user-supplied TLS Secret
// in the operator namespace so that the proxy StatefulSet can mount it.
func (a *tailscaleSTSReconciler) syncTLSSecret(ctx context.Context, sts *tailscaleSTSConfig, hsvc *corev1.Service) error {
	// Shared-proxy multi-host mode: create one secret per hostname.
	if len(sts.CustomTLSCerts) > 0 {
		return a.syncTLSSecretsMulti(ctx, sts, hsvc)
	}

	// Single-host mode (original behavior).
	if len(sts.CustomTLSData) == 0 {
		return nil
	}
	secretName := hsvc.Name + "-tls"
	tlsLabels := make(map[string]string, len(sts.ChildResourceLabels)+1)
	for k, v := range sts.ChildResourceLabels {
		tlsLabels[k] = v
	}
	tlsLabels["tailscale.com/secret-type"] = "custom-tls"
	sec := &corev1.Secret{
		...
		Data: sts.CustomTLSData,
	}
	existing := &corev1.Secret{}
	if err := a.Get(ctx, types.NamespacedName{Name: secretName, Namespace: a.operatorNamespace}, existing); err != nil {
		if !apierrors.IsNotFound(err) {
			return fmt.Errorf("getting TLS secret: %w", err)
		}
		return a.Create(ctx, sec)
	}
	existing.Data = sts.CustomTLSData
	return a.Update(ctx, existing)
}

// syncTLSSecretsMulti creates (or updates) one Kubernetes Secret per hostname
// for shared-proxy mode. Each secret is named <hsvc>-tls-<sanitized-hostname>.
func (a *tailscaleSTSReconciler) syncTLSSecretsMulti(ctx context.Context, sts *tailscaleSTSConfig, hsvc *corev1.Service) error {
	tlsLabels := make(map[string]string, len(sts.ChildResourceLabels)+1)
	for k, v := range sts.ChildResourceLabels {
		tlsLabels[k] = v
	}
	...
	for hostname, data := range sts.CustomTLSCerts {
		secretName := sharedTLSSecretName(hsvc.Name, hostname)
		perHostLabels := make(map[string]string, len(tlsLabels)+1)
		for k, v := range tlsLabels {
			perHostLabels[k] = v
		}
		perHostLabels["tailscale.com/tls-hostname"] = sanitizeHostnameForLabel(hostname)

		sec := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name:      secretName,
				Namespace: a.operatorNamespace,
				Labels:    perHostLabels,
			},
			Type: corev1.SecretTypeTLS,
			Data: data,
		}
		...
		if err := a.Get(ctx, types.NamespacedName{Name: secretName, Namespace: a.operatorNamespace}, existing); err != nil {
			if !apierrors.IsNotFound(err) {
				return fmt.Errorf("getting TLS secret for %s: %w", hostname, err)
			}
			if err := a.Create(ctx, sec); err != nil {
				return fmt.Errorf("creating TLS secret for %s: %w", hostname, err)
			}
		} else {
			existing.Data = data
			if err := a.Update(ctx, existing); err != nil {
				return fmt.Errorf("updating TLS secret for %s: %w", hostname, err)
			}
		}
	}

	// Clean up secrets for hostnames that are no longer in the group.
	var existingSecrets corev1.SecretList
	if err := a.List(ctx, &existingSecrets,
		client.InNamespace(a.operatorNamespace),
		client.MatchingLabels(tlsLabels)); err != nil {
		return fmt.Errorf("listing existing TLS secrets: %w", err)
	}
	for _, sec := range existingSecrets.Items {
		hostnameLabel := sec.Labels["tailscale.com/tls-hostname"]
		if hostnameLabel == "" {
			continue
		}
		// Check if this hostname is still in the set.
		stillPresent := false
		for hostname := range sts.CustomTLSCerts {
			if sanitizeHostnameForLabel(hostname) == hostnameLabel {
				stillPresent = true
				break
			}
		}
		if !stillPresent {
			if err := a.Delete(ctx, &sec); err != nil && !apierrors.IsNotFound(err) {
				return fmt.Errorf("deleting stale TLS secret %s: %w", sec.Name, err)
			}
		}
	}
...
// sharedTLSSecretName returns the Kubernetes Secret name for a given hostname
// in shared-proxy mode.
func sharedTLSSecretName(hsvcName, hostname string) string {
	sanitized := sanitizeHostnameForLabel(hostname)
	name := hsvcName + "-tls-" + sanitized
	// Kubernetes name limit is 253 chars; truncate if needed.
	if len(name) > 253 {
		name = name[:253]
	}
	return name
...
// sanitizeHostnameForLabel converts a hostname (which may contain dots) into a
// string safe for use as a Kubernetes label value (max 63 chars, alphanumeric +
// dashes).
func sanitizeHostnameForLabel(hostname string) string {
...
func (a *tailscaleSTSReconciler) Provision(ctx context.Context, logger *zap.SugaredLogger, sts *tailscaleSTSConfig) (*corev1.Service, error) {
	...
	if err := a.syncTLSSecret(ctx, sts, hsvc); err != nil {
		return nil, fmt.Errorf("failed to sync TLS secret: %w", err)
	}
...
func (a *tailscaleSTSReconciler) provisionSecrets(ctx context.Context, tailscaleClient tsClient, loginUrl string, stsC *tailscaleSTSConfig, hsvc *corev1.Service, logger *zap.SugaredLogger) ([]string, error) {
	...
	for _, secret := range secrets.Items {
		...
		if _, err := fmt.Sscanf(secret.Name, hsvc.Name+"-%d", &ordinal); err != nil {
			continue // skip secrets that don't match the ordinal naming pattern (e.g., TLS secrets)
		}
...
func (a *tailscaleSTSReconciler) reconcileSTS(ctx context.Context, logger *zap.SugaredLogger, sts *tailscaleSTSConfig, headlessSvc *corev1.Service, proxySecrets []string) (*appsv1.StatefulSet, error) {
	...
	// Mount custom TLS secret(s).
	if len(sts.CustomTLSCerts) > 0 {
		// Shared-proxy mode: mount one secret per hostname at /etc/tailscaled-tls/<hostname>/
		// Sort hostnames for deterministic volume ordering (prevents unnecessary STS updates).
		sortedHosts := make([]string, 0, len(sts.CustomTLSCerts))
		for hostname := range sts.CustomTLSCerts {
			sortedHosts = append(sortedHosts, hostname)
		}
		slices.Sort(sortedHosts)
		idx := 0
		for _, hostname := range sortedHosts {
			secretName := sharedTLSSecretName(headlessSvc.Name, hostname)
			volName := fmt.Sprintf("custom-tls-%d", idx)
			...
			container.VolumeMounts = append(container.VolumeMounts, corev1.VolumeMount{
				Name:      volName,
				ReadOnly:  true,
				MountPath: fmt.Sprintf("/etc/tailscaled-tls/%s", hostname),
			})
			idx++
		}
	} else if len(sts.CustomTLSData) > 0 {
		// Single-Ingress mode: mount one secret at /etc/tailscaled-tls/
		tlsSecretName := headlessSvc.Name + "-tls"
		pod.Spec.Volumes = append(pod.Spec.Volumes, corev1.Volume{
			Name: "custom-tls",
			VolumeSource: corev1.VolumeSource{
				Secret: &corev1.SecretVolumeSource{
