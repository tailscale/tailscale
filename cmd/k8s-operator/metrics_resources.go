...
func metricsResourceLabels(opts *metricsOpts) map[string]string {
	lbls := map[string]string{
		...
		labelPromProxyParentName: reconciler.TruncateLabelValue(opts.proxyLabels[LabelParentName]),
	}
	// Include namespace label for proxies created for a namespaced type.
	if isNamespacedProxyType(opts.proxyType) {
		lbls[labelPromProxyParentNamespace] = reconciler.TruncateLabelValue(opts.proxyLabels[LabelParentNamespace])
	}
	lbls[labelPromJob] = reconciler.TruncateLabelValue(promJobName(opts))
	return lbls
}
...
func metricsSvcSelector(proxyLabels map[string]string, proxyType string) map[string]string {
	sel := map[string]string{
		...
		labelPromProxyParentName: reconciler.TruncateLabelValue(proxyLabels[LabelParentName]),
	}
	// Include namespace label for proxies created for a namespaced type.
	if isNamespacedProxyType(proxyType) {
		sel[labelPromProxyParentNamespace] = reconciler.TruncateLabelValue(proxyLabels[LabelParentNamespace])
	}
	return sel
}
