...
func TestServicePGReconciler_CleanupOnAnnotationRemoved(t *testing.T) {
	...
	got := &corev1.Service{}
	if err := fc.Get(context.Background(), types.NamespacedName{Namespace: svc.Namespace, Name: svc.Name}, got); err != nil {
	...
	if _, err := ft.VIPServices().Get(context.Background(), fmt.Sprintf("svc:default-%s", svc.Name)); err == nil {
