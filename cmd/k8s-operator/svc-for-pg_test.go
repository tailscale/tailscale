...
func TestServicePGReconciler_UpdateHostname(t *testing.T) {
	...
	// NOTE: we need to update hte ingress config secret because there is no containerboot in the fake proxy pod
...
func setupServiceTest(t *testing.T) (*HAServiceReconciler, *corev1.Secret, client.Client, *fakeTSClient) {
	...
	// NOTE: not sure if we need to setup the state secret here...
