...
// TestAuthorizeStrictMode verifies OAuth authorization endpoint security in strict mode.
// In strict mode, the authorization endpoint must:
// - Require a valid client_id parameter that corresponds to a registered client
// - Require a redirect_uri that exactly matches the registered client's URI
// - Only accept requests from funnel clients (registered OAuth applications)
// - Reject requests with missing or invalid client parameters
func TestAuthorizeStrictMode(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "strict mode - valid client and redirect URI (funnel request)",
			strictMode:     true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// In production, strict mode works with both funnel and non-funnel requests.
			if tt.strictMode {
