...
func newUpFlagSet(goos string, upArgs *upArgsT, cmd string) *flag.FlagSet {
	...
	upf.StringVar(&upArgs.audience, "audience", "", "Audience used to generate authkeys via workload identity federation")
...
func runUp(ctx context.Context, cmd string, args []string, upArgs upArgsT) (retErr error) {
	...
	if simpleUp {
		...
		if f, ok := tailscale.HookResolveAuthKeyViaWIF.GetOk(); ok && authKey == "" {
			...
			authKey, err = f(ctx, prefs.ControlURL, upArgs.clientID, idToken, audience, strings.Split(upArgs.advertiseTags, ","))
			if err != nil {
				return err
			}
		}
...
func preflessFlag(flagName string) bool {
	switch flagName {
	case "auth-key", "force-reauth", "reset", "qr", "qr-format", "json", "timeout", "accept-risk", "host-routes", "client-id", "client-secret", "id-token", "audience":
		return true
	}
	return false
}
