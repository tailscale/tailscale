...
func (e *serveEnv) runServeSetConfig(ctx context.Context, args []string) (err error) {
	...
	unadvertisedServices := set.Set[string]{}
	...
	for name, details := range scf.Services {
		...
		if v, set := details.Advertised.Get(); !set || v {
			...
			unadvertisedServices.Add(name.String())
		}
	}
	...
	if e.allServices {
		...
		for _, svc := range prefs.AdvertiseServices {
			if !unadvertisedServices.Contains(svc) {
				servicesList = append(servicesList, svc)
			}
		}
		servicesList = append(servicesList, advertisedServices.Slice()...)
	}
	_, err = e.lc.EditPrefs(ctx, &ipn.MaskedPrefs{
		AdvertiseServicesSet: true,
		Prefs: ipn.Prefs{
