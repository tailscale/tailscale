...
func run() error {
	...
	if cfg.InKubernetes {
		...
		// Clear out any state from previous runs of containerboot. We know
		// we're in kube, but that doesn't guarantee the state store is
		// properly configured.
		if hasKubeStateStore(cfg) {
			if err := kc.resetContainerbootState(bootCtx, cfg.PodUID); err != nil {
				return fmt.Errorf("error clearing previous containerboot state: %w", err)
			}
		}
	}
