...
func configFromEnv() (*settings, error) {
	cfg := &settings{
		AuthKey:            def.GetenvFirst([]string{"TS_AUTHKEY", "TS_AUTH_KEY"}, ""),
		ClientID:           def.Getenv("TS_CLIENT_ID", ""),
		ClientSecret:       def.Getenv("TS_CLIENT_SECRET", ""),
		IDToken:            def.Getenv("TS_ID_TOKEN", ""),
		Audience:           def.Getenv("TS_AUDIENCE", ""),
		Hostname:           def.Getenv("TS_HOSTNAME", ""),
		Routes:             defaultEnvStringPointer("TS_ROUTES"),
		ServeConfigPath:    def.Getenv("TS_SERVE_CONFIG", ""),
		ProxyTargetIP:      def.Getenv("TS_DEST_IP", ""),
		ProxyTargetDNSName: def.Getenv("TS_EXPERIMENTAL_DEST_DNS_NAME", ""),
		TailnetTargetIP:    def.Getenv("TS_TAILNET_TARGET_IP", ""),
		TailnetTargetFQDN:  def.Getenv("TS_TAILNET_TARGET_FQDN", ""),
		DaemonExtraArgs:    def.Getenv("TS_TAILSCALED_EXTRA_ARGS", ""),
		ExtraArgs:          def.Getenv("TS_EXTRA_ARGS", ""),
		InKubernetes:       os.Getenv("KUBERNETES_SERVICE_HOST") != "",
		UserspaceMode:      def.GetenvBool("TS_USERSPACE", true),
		StateDir:           def.Getenv("TS_STATE_DIR", ""),
		AcceptDNS:          defaultEnvBoolPointer("TS_ACCEPT_DNS"),
		KubeSecret: func() string {
			if os.Getenv("KUBERNETES_SERVICE_HOST") != "" {
				return def.Getenv("TS_KUBE_SECRET", "tailscale")
			}
			return def.Getenv("TS_KUBE_SECRET", "")
		}(),
		SOCKSProxyAddr:                        def.Getenv("TS_SOCKS5_SERVER", ""),
		HTTPProxyAddr:                         def.Getenv("TS_OUTBOUND_HTTP_PROXY_LISTEN", ""),
		Socket:                                def.Getenv("TS_SOCKET", "/tmp/tailscaled.sock"),
		AuthOnce:                              def.GetenvBool("TS_AUTH_ONCE", false),
		Root:                                  def.Getenv("TS_TEST_ONLY_ROOT", "/"),
		...
		PodIP:                                 def.Getenv("POD_IP", ""),
		EnableForwardingOptimizations:         def.GetenvBool("TS_EXPERIMENTAL_ENABLE_FORWARDING_OPTIMIZATIONS", false),
		HealthCheckAddrPort:                   def.Getenv("TS_HEALTHCHECK_ADDR_PORT", ""),
		LocalAddrPort:                         def.Getenv("TS_LOCAL_ADDR_PORT", "[::]:9002"),
		MetricsEnabled:                        def.GetenvBool("TS_ENABLE_METRICS", false),
		HealthCheckEnabled:                    def.GetenvBool("TS_ENABLE_HEALTH_CHECK", false),
		DebugAddrPort:                         def.Getenv("TS_DEBUG_ADDR_PORT", ""),
		EgressProxiesCfgPath:                  def.Getenv("TS_EGRESS_PROXIES_CONFIG_PATH", ""),
		IngressProxiesCfgPath:                 def.Getenv("TS_INGRESS_PROXIES_CONFIG_PATH", ""),
		PodUID:                                def.Getenv("POD_UID", ""),
	}
