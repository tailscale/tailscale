...
func TestRunIPNBusWatcherReconnects(t *testing.T) {
	...
	go func() {
		defer close(done)
		runIPNBusWatcher(ctx, watch, ipn.NotifyInitialStatus|ipn.NotifyNoNetMap, notify)
	}()
	...
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for replacement status")
	}
	cancel()
	select {
	case <-done:
	...
	for i, got := range masks[:2] {
		if want := ipn.NotifyInitialStatus | ipn.NotifyNoNetMap; got != want {
			t.Errorf("watch attempt %d mask = %v, want %v", i, got, want)
		}
	}
}
