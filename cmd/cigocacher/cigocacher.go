...
// cigocacher is an opinionated-to-Tailscale client for gocached.
package main
...
// func httpTimeout(bytes int64) time.Duration {
// 	const (
// 		MiB = 1 << 20
// 		GiB = 1 << 30
// 	)

// 	// Capped at 1 minute for very large files, and to keep the logic below
// 	// simple (no need to worry about integer overflow).
// 	if bytes > GiB {
// 		return time.Minute
// 	}

// 	// Allow at least 5s per file, but scale up for large files. Every transfer
// 	// that averages at least 1 MiB/s will complete within this time.
// 	sizeAdjustedTimeout := (time.Duration(bytes) * time.Second) / MiB
// 	return max(5*time.Second, sizeAdjustedTimeout)
// }
