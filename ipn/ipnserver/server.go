...
func serveDebugHandler(w http.ResponseWriter, r *http.Request, logid string, opts Options, b *ipn.LocalBackend, e wgengine.Engine) {
	...
	f("<html><body><h1>IPN state</h1><h2>Run args</h2>")
	f("<p><b>logid:</b> %s</p>\n", logid)
	f("<p><b>opts:</b> <code>%s</code></p>\n", html.EscapeString(fmt.Sprintf("%+v", opts)))

	st := b.Status()
	f("<table border=1 cellpadding=5><tr><th>Peer</th><th>HostName</th><th>Rx</th><th>Tx</th><th>Handshake</th><th>Addrs</th></tr>")
	now := time.Now()
	longAgo := now.Add(-15 * time.Minute) // server rounds to 10 minutes
	for _, peer := range st.Peers() {
		ps := st.Peer[peer]
		var hsAgo string
		if !ps.LastHandshake.IsZero() {
			hsAgo = now.Sub(ps.LastHandshake).Round(time.Second).String()
		} else {
		...
		f("<tr><td>%s</td><td>%s</td><td>%v</td><td>%v</td><td>%v</td>",
			peer.ShortString(),
