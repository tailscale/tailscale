...
type AuditLogger struct {
	...
	flusherStopped  chan struct{}      // signal to indicate the flush worker has stopped
...
func (al *AuditLogger) flush(op flushOp) {
	...
	if (al.flushWorkerDone == nil) || op.transport == nil || al.logID == "" {
		al.mu.Unlock()
		return
	}
...
// appendToStoreLocked persists logs to the store that are
...
var _ LogStore = (*StateStore)(nil)

// StateStore is a concrete implementation of LogStore
...
func NewStateStore(store ipn.StateStore, logf logger.Logf) LogStore {
	return &StateStore{
		store: store,
		logf:  logf,
	}
...
func (s *StateStore) generateKey(key ipn.ProfileID) string {
...
func (s *StateStore) Save(key ipn.ProfileID, txns []auditLogTxn) error {
...
func (s *StateStore) Load(key ipn.ProfileID) ([]auditLogTxn, error) {
