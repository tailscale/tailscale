...
// Package ipntest provides utilities for black-box testing of LocalAPI ([ipnserver]).
package ipntest
...
func NewUnstartedServer(tb testing.TB, opts ...Option) *Server {
	...
	s.httpServer.Config.ErrorLog = logger.StdLogger(logger.WithPrefix(options.Logf(), "ipntest: "))
	tb.Cleanup(s.Close)
	return s
}
...
// BlockWhileInUse blocks while the specified actor can't connect to the server
