...
func TestWriteTLSCertAndKey(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "cert_share_mode_update_existing",
			initial: map[string][]byte{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.certShareMode {
				envknob.Setenv("TS_CERT_SHARE_MODE", "rw")
			} else {
...
func TestReadTLSCertAndKey(t *testing.T) {
	...
	tests := []struct {
		...
		isReadOnlyMode bool                    // whether to run in cert-share read-only mode
		domain         string
