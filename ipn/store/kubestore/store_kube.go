...
func newWithClient(logf logger.Logf, c kubeclient.Client, secretName string) (*Store, error) {
	...
	if envknob.IsCertShareReadWriteMode() || envknob.IsCertShareReadOnlyMode() {
...
func (s *Store) WriteTLSCertAndKey(domain string, cert, key []byte) (err error) {
	defer func() {
		if err == nil {
			s.memory.WriteState(ipn.StateKey(sanitizeKey(domain+".crt")), cert)
			s.memory.WriteState(ipn.StateKey(sanitizeKey(domain+".key")), key)
		}
	}()
