...
func (b *LocalBackend) TailFSSetShare(share *taildrive.Share) error {
	var err error
...
func (b *LocalBackend) tailFSSetShareLocked(share *taildrive.Share) (views.SliceView[*taildrive.Share, taildrive.ShareView], error) {
...
func (b *LocalBackend) tailFSRenameShareLocked(oldName, newName string) (views.SliceView[*taildrive.Share, taildrive.ShareView], error) {
	existingShares := b.pm.prefs.TailFSShares()
	...
	slices.SortFunc(shares, taildrive.CompareShares)
	err := b.tailFSSetSharesLocked(shares)
	if err != nil {
		return existingShares, err
	}
...
func (b *LocalBackend) tailFSRemoveShareLocked(name string) (views.SliceView[*taildrive.Share, taildrive.ShareView], error) {
	existingShares := b.pm.prefs.TailFSShares()
...
func (b *LocalBackend) tailFSSetSharesLocked(shares []*taildrive.Share) error {
...
func (b *LocalBackend) tailFSNotifyShares(shares views.SliceView[*taildrive.Share, taildrive.ShareView]) {
	// Ensures shares is not nil to distinguish "no shares" from "not notifying shares"
	if shares.IsNil() {
		shares = views.SliceOfViews(make([]*taildrive.Share, 0))
	}
...
func (b *LocalBackend) tailFSNotifyCurrentSharesLocked() {
	var shares views.SliceView[*taildrive.Share, taildrive.ShareView]
...
func tailFSShareViewsEqual(a *views.SliceView[*taildrive.Share, taildrive.ShareView], b views.SliceView[*taildrive.Share, taildrive.ShareView]) bool {
	if a == nil {
		return false
	}
	...
	for i := 0; i < a.Len(); i++ {
		if !taildrive.ShareViewsEqual(a.At(i), b.At(i)) {
			return false
		}
	}
...
func (b *LocalBackend) TailFSGetShares() views.SliceView[*taildrive.Share, taildrive.ShareView] {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *LocalBackend) updateTailFSPeersLocked(nm *netmap.NetworkMap) {
	...
	var tailFSRemotes []*taildrive.Remote
...
func (b *LocalBackend) tailFSRemotesFromPeers(nm *netmap.NetworkMap) []*taildrive.Remote {
	tailFSRemotes := make([]*taildrive.Remote, 0, len(nm.Peers))
	for _, p := range nm.Peers {
		...
		tailFSRemotes = append(tailFSRemotes, &taildrive.Remote{
