...
// needIngressHintLocked reports whether the node is in a state where funnel is not actively enabled, but it seems that
// it is intended to be used with funnel.
func (b *LocalBackend) needIngressHintLocked() bool {
...
func (b *LocalBackend) applyPrefsToHostinfoLocked(hi *tailcfg.Hostinfo, prefs ipn.PrefsView) {
	...
	// The Hostinfo.WantIngress field tells control whether the user intends to use funnel with this node even
	// though it is not currently enabled. This is an optimization to control- Funnel requires creation of DNS
	// records and because DNS propagation can take time, we want to ensure that the records exist for any node that
	// intends to use funnel even if it's not enabled. If hi.IngressEnabled is true, control knows that DNS records
	// are needed, so we can save bandwidth and not send WireIngress.
	hi.WireIngress = b.needIngressHintLocked()
...
func (b *LocalBackend) updateIngressLocked() {
	...
	if wire := b.needIngressHintLocked(); b.hostinfo.WireIngress != wire {
