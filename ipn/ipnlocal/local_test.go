...
func TestTailFSManageShares(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			add: &taildrive.Share{Name: "  E  "},
			expect: []*taildrive.Share{
				{Name: "b"},
				{Name: "d"},
				{Name: "e"},
			},
		...
		},
		{
			...
			add: &taildrive.Share{Name: "  B  ", Path: "ii"},
			expect: []*taildrive.Share{
				{Name: "b", Path: "ii"},
				{Name: "d"},
			},
	...
	t.Cleanup(func() {
		taildrive.DisallowShareAs = false
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.disabled {
				...
				b.sys.Set(taildriveimpl.NewFileSystemForRemote(b.logf))
			}
			b.mu.Unlock()
			...
			result := make(chan views.SliceView[*taildrive.Share, taildrive.ShareView], 1)
