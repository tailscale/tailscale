...
// RunningGoroutines returns the number of currently-running goroutines
// tracked by b's goroutine tracker.
func (f forTest) RunningGoroutines() int64 { return f.b.goTracker.RunningGoroutines() }

// StartedGoroutines returns the total number of goroutines started by
// b's goroutine tracker since b was created.
func (f forTest) StartedGoroutines() int64 { return f.b.goTracker.StartedGoroutines() }

// AddGoroutineDoneCallback registers cb to be invoked each time a
// goroutine tracked by b finishes. The returned func unregisters cb.
func (f forTest) AddGoroutineDoneCallback(cb func()) (unregister func()) {
	return f.b.goTracker.AddDoneCallback(cb)
}
