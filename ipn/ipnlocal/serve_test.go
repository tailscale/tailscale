...
func TestServeHTTPProxyGrantHeader(t *testing.T) {
	...
	// Configure packet filter with one cap grant equivalent to
	// "grants": [
	//		{
	//			"src": ["100.150.151.152"], # user-owned node
	//			"dst": ["100.150.151.151"],
	//			"app": {"example.com/cap/some-app": [{"role": "🐿️"}]},
	//		},
	//		{
	//			"src": ["100.150.151.153"], # tagged node
	//			"dst": ["100.150.151.151"],
	//			"app": {"example.com/cap/some-app": [{"role": "Viewer"}]},
	//		},
	// ]
	nm := b.NetMap()
	matches, err := filter.MatchesFromFilterRules([]tailcfg.FilterRule{
		{
			...
			CapGrant: []tailcfg.CapGrant{{
				...
				CapMap: tailcfg.PeerCapMap{
					"example.com/cap/some-app": []tailcfg.RawMessage{
						`{"role": "🐿"}`,
					},
				},
			}},
	...
	tests := []struct {
	...
	}{
		{
			...
			wantHeaders: []headerCheck{
				...
				{"Tailscale-User-Profile-Pic", "https://example.com/photo.jpg"},
				{"Tailscale-Headers-Info", "https://tailscale.com/s/serve-headers"},
				{"Tailscale-User-Capabilities", `{"example.com/cap/some-app":[{"role":"🐿"}]}`},
			},
		},
		{
			name:  "request-from-tagged-node-within-tailnet",
			...
			wantHeaders: []headerCheck{
				...
				{"Tailscale-User-Profile-Pic", ""},
				{"Tailscale-Headers-Info", ""},
				{"Tailscale-User-Capabilities", `{"example.com/cap/some-app":[{"role":"Viewer"}]}`},
			},
		},
		{
			name:  "request-from-outside-tailnet",
...
func TestSerialisePeerCapMap(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			capMap: tailcfg.PeerCapMap{
				"foo.com/cap/some-app": []tailcfg.RawMessage{
					`{"role": "Admin"}`,
				},
				"bar.com/cap/some-other-app": []tailcfg.RawMessage{
					`{"role": "Viewer"}`,
				},
			},
			maxNumBytes: 50,
			...
			wantOneOfSerialized: []string{
				`{"foo.com/cap/some-app":[{"role":"Admin"}],"bar.com/cap/some-other-app":[{"role":"Viewer"}]}`,
				`{"bar.com/cap/some-other-app":[{"role":"Viewer"}],"foo.com/cap/some-app":[{"role":"Admin"}]}`,
			},
			wantTruncated: false,
		},
		{
			...
			wantOneOfSerialized: []string{
				`{"foo.com/cap/some-app":[{"role":"Admin"}]}`,
				`{"bar.com/cap/some-other-app":[{"role":"Viewer"}]}`,
			},
			wantTruncated: true,
		},
	}
