...
func TestServeHTTPProxyGrantHeaders(t *testing.T) {
	b := newTestBackend(t)

	// Configure packet filter with one cap grant
	// that is the equivalent of
	// "grants": [
	//		{
	//			"src": ["100.150.151.152"],
	//			"dst": ["100.150.151.151"],
	//			"app": {"example.com/cap/grafana": [{"role": "Admin"}]},
	//		}
	// ]
	nm := b.NetMap()
	...
	tests := []struct {
	...
	}{
		{
			...
			wantHeaders: []headerCheck{
				...
				{"Tailscale-User-Profile-Pic", "https://example.com/photo.jpg"},
				{"Tailscale-Headers-Info", "https://tailscale.com/s/serve-headers"},
				{"Tailscale-User-Capabilities", `{"example.com/cap/grafana":[{"role":"Admin"}]}`},
			},
		},
		{
			name:  "request-from-tagged-node-within-tailnet",
