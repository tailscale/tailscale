...
func closePolicyStore(store source.Store) {
...
func (e *desktopSessionsExt) initUserPolicyStore(session *Session) func() {
	...
	if err := windows.WTSQueryUserToken(uint32(session.ID), &token); err != nil {
		e.logf("failed to get user token for session %d (uid %s): %v", session.ID, uid, err)
		return nil
	}
	defer token.Close()
