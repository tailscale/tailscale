...
type Status struct {
	BackendState string
	IPs          []string // Tailscale IP(s) assigned to this node
...
func (sb *StatusBuilder) AddIP(ip netaddr.IP) {
	sb.mu.Lock()
	defer sb.mu.Unlock()
	if sb.locked {
	...
	sb.st.IPs = append(sb.st.IPs, ip.String())
}
...
`)
	...
	f("<p>Tailscale IP: %s", strings.Join(st.IPs, ", "))
