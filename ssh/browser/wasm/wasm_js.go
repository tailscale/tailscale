...
// Package that generates JS bindings for Tailscale via WebAssembly.
//
...
// start(callbacks), login(), logout(), and ssh(...).
package main
...
func newIPN(jsConfig js.Value) map[string]any {
	...
	return map[string]any{
		...
		"ssh": js.FuncOf(func(this js.Value, args []js.Value) interface{} {
			if len(args) != 6 {
				log.Printf("Usage: ssh(hostname, writeFn, readFn, row, cols, onDone)")
				return nil
			}
			go jsIPN.ssh(
				args[0].String(),
