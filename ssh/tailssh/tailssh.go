...
func (c *conn) clientAuth(cm gossh.ConnMetadata) (perms *gossh.Permissions, retErr error) {
	...
	case accepted:
		// do nothing
	case rejectedUser:
		return nil, c.errBanner("tailnet policy does not permit you to SSH as user '"+c.info.sshUser+"'", nil)
	default:
...
func (c *conn) matchRule(r *tailcfg.SSHRule) (a *tailcfg.SSHAction, localUser string, acceptEnv []string, err error) {
	...
	if !r.Action.Reject {
		...
		if localUser == "" {
			userMatchError = errUserMatch
		}
	}
	if userMatchError == nil && !c.anyPrincipalMatches(r.Principals) {
		return nil, "", nil, errPrincipalMatch
	}
	if userMatchError != nil {
		return nil, "", nil, userMatchError
	}
