...
main() {
	...
	if [ -f /etc/os-release ]; then
		...
		#  - ID_LIKE: space-separated list of distros this one is derived
		#    from, if any (e.g. "ubuntu debian"), used as a fallback when
		#    ID itself isn't recognized.
		. /etc/os-release
		VERSION_MAJOR="${VERSION_ID:-}"
		VERSION_MAJOR="${VERSION_MAJOR%%.*}"
		detect_os_by_id "$ID"
		# If the distro wasn't recognized directly, fall back to the
		# distro(s) it declares compatibility with via ID_LIKE.
		if [ -z "$OS" ] && [ -n "${ID_LIKE:-}" ]; then
			for id_like in $ID_LIKE; do
				detect_os_by_id "$id_like"
				if [ -n "$OS" ]; then
					break
				fi
			done
