...
//
// Implementation note: Publisher[T] is a thin user-facing facade
// over a non-generic *publisherCore. Carrying T on the public type
// preserves the typed API of Publish(v T), but all of the actual
// state (the *Client back-pointer, the stop flag, and the cached
// reflect.Type used by diagnostic introspection) lives on the core
// and is not duplicated per T.
//
// The diagnostic surface that motivates the publisher interface
// (Debugger.PublishTypes) is served by *publisherCore directly, so
// adding new typed publishers does not pay an itab+dictionary cost
// just to satisfy diagnostic enumeration.
