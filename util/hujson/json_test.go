...
func Test(t *testing.T) {
	tests := []struct {
		...
		want: Value{Value: &Object{
			Members: [][2]Value{{
				{StartOffset: 1, Value: Literal(`"k"`), EndOffset: 4},
				{StartOffset: 5, Value: Literal(`"v"`), EndOffset: 8},
			}},
		}},
		...
		want: Value{
			...
			Value: &Object{
				Members: [][2]Value{{
					{BeforeExtra: Extra(" "), StartOffset: 3, Value: Literal(`"k"`), EndOffset: 6, AfterExtra: Extra(" ")},
					{BeforeExtra: Extra(" "), StartOffset: 9, Value: Literal(`"v"`), EndOffset: 12, AfterExtra: Extra(" ")},
				}},
			},
			EndOffset:  14,
			AfterExtra: Extra(" "),
		...
		want: Value{
			...
			Value: &Array{
				Elements: []Value{
					...
					{BeforeExtra: Extra(" /**/ "), StartOffset: 115, Value: &Array{}, EndOffset: 117, AfterExtra: Extra(" /**/ ")},
				},
			},
			EndOffset:  124,
			AfterExtra: Extra(" /**/ "),
	...
	for _, tt := range tests {
		...
		if diff := cmp.Diff(tt.want, gotVal); diff != "" {
		...
		if gotErr == nil {
			...
			if tt.wantMin != "" {
				...
				if diff := cmp.Diff(wantMinVal, gotMinVal); diff != "" {
			...
			if tt.wantStd != "" {
				...
				if diff := cmp.Diff(wantStdVal, gotStdVal); diff != "" {
