...
// Mode is which platform-specific strategy to use (direct vs Android SAF).
...
func (m *Manager) PutFile(
	...
	var dstPath string
	if m.opts.PutMode == ipn.PutModeDirect {
		var err error
	...
	wc, partialPath, err := m.openWriterAndPaths(id, m.opts.PutMode, inFile, baseName, dstPath, offset)
	if err != nil {
...
func (m *Manager) finalizeSAF(
	...
	for i := 0; i < maxTries; i++ {
		newURI, err := m.fileOps.RenamePartialFile(partialPath, m.opts.Dir, name)
		if err != nil {
			return err
		}
