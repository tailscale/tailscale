...
type Conn struct {
	...
	canCreateEPUnlocked syncs.AtomicBool // whether mu is held; for unfortunate CreateEndpoint use
...
type Options struct {
	...
	// This may end up calling Conn.CreateEndpoint.
...
// e is a discovery-capable peer and it's been awhile since we last
// called the hook.
//
...
func (c *Conn) handleDiscoMessage(msg []byte, src netaddr.IPPort) bool {
	...
	if !ok {
		...
		// noteRecvActivity calls back into CreateEndpoint, which we can't easily
		// control, and CreateEndpoint expects to be called with c.mu held, but we
		// hold it here, and it's too invasive for now to release it here and
		// recheck invariants.  So instead, use this unfortunate hack: set muHeld
		// which CreateEndpoint then checks to conditionally acquire the mutex. I'm
		// so sorry.
