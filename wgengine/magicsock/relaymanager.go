...
func (r *relayManager) handleRxHandshakeDiscoMsgRunLoop(event relayHandshakeDiscoMsgEvent) {
	...
	case *disco.Ping:
		...
		//
		// The endpoint.go counterpart code makes efforts to suppress duplicate
		// pongs where the same ping can be received both via raw socket and UDP
		// socket on Linux. We make no such efforts here as the raw socket BPF
		// program does not support Geneve-encapsulated disco. It is also
		// disabled by default.
		...
		vni.set(work.se.VNI)
		go event.conn.sendDiscoMessage(epAddr{ap: event.from, vni: vni}, work.ep.publicKey, epDisco.key, &disco.Pong{
			TxID: msg.TxID,
			Src:  event.from,
		}, discoVerboseLog)
