...
func (m *webrtcManager) handleConnectionStateChange(ps *webrtcPeerState, state webrtc.PeerConnectionState) {
	...
	case webrtc.PeerConnectionStateConnected:
		...
		// Update remoteAddr from the *actually selected* ICE candidate pair,
		// not from whatever candidate was last signaled. This ensures the
		// address shown in "tailscale status" reflects the real data path.
		go func() {
			...
			if addr, err := netip.ParseAddr(cp.Remote.Address); err == nil {
				m.mu.Lock()
				ps.remoteAddr = netip.AddrPortFrom(addr, uint16(cp.Remote.Port))
				m.mu.Unlock()
			}
