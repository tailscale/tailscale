...
func rdnsNameToIPv4(name []byte) (ip netaddr.IP, ok bool) {
	// More than bytes to detect overflow.
	var b [4]uint16

	part := 0
	partEmpty := true
	// This ranges over 4.3.2.1. in 4.3.2.1.ip-addr.arpa
	for _, c := range name {
		// There are four dot-separated parts.
		if part == 4 {
			break
		}
		if c == '.' {
			// No part can be empty, such as in 4..2.1.
			if partEmpty {
		...
		// Only decimal digits in each part.
		if !('0' <= c && c <= '9') {
			return netaddr.IP{}, false
		}
		b[part] = b[part]*10 + uint16(c-'0')
		partEmpty = false
		// Each part is a byte, so e.g. 400.300.200.100 is not allowed.
		if b[part] >= 256 {
			return netaddr.IP{}, false
		}
	}

	return netaddr.IPv4(byte(b[3]), byte(b[2]), byte(b[1]), byte(b[0])), true
}
...
func rdnsNameToIPv6(name []byte) (ip netaddr.IP, ok bool) {
	...
	prevDot := false
	// i ranges over name backwards; j ranges over b forward.
