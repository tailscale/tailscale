...
func (ns *Impl) acceptTCP(r *tcp.ForwarderRequest) {
	...
	hittingDNS := reqDetails.LocalPort == 53
	hittingTailfs := ns.tailfsForLocal != nil && reqDetails.LocalPort == ipnlocal.TailfsInternalPort
	if hittingServiceIP && (hittingDNS || hittingTailfs) {
		c := getConnOrReset()
		if c == nil {
			return
		}
		addrPort := netip.AddrPortFrom(clientRemoteIP, reqDetails.RemotePort)
		if reqDetails.LocalPort == 53 {
			// DNS
			go ns.dns.HandleTCPConn(c, addrPort)
		} else {
			// TailFS WebDAV
			// TODO(oxtoacart): move TailFS to port 80 on its own IP maybe?
			if err := ns.tailfsForLocal.HandleConn(c, net.TCPAddrFromAddrPort(addrPort)); err != nil {
