...
func (ns *Impl) forwardTCP(client *gonet.TCPConn, clientRemoteIP netaddr.IP, wq *waiter.Queue, dialAddr netaddr.IPPort) {
	...
	go func() {
		<-ctx.Done()
		// Inform the server we won't read anymore, EOF will happen.
		server.CloseRead()
		// Shutdown the client, but don't close it, otherwise buffers would be dropped.
		client.CloseRead()
		client.CloseWrite()
	}()
