...
func (e *userspaceEngine) noteReceiveActivity(dk tailcfg.DiscoKey) {
	...
	// If the last activity was over a minute ago (or
	// never), then see if we need to reprogram Wireguard.
	if was.Before(now.Add(-time.Minute)) {
		e.logf("noting now-active peer %v", dk.ShortString())
...
// this simply returns the zero value.
