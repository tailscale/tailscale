...
// TestInboundTSMPAppConnectorTransitIPRecordsAndContinues verifies that an
// unmapped app-connector transit IP is recorded with its specific reason while
// the TSMP packet remains accepted for tstun's mapping-recovery handler.
func TestInboundTSMPAppConnectorTransitIPRecordsAndContinues(t *testing.T) {
	t.Parallel()
	var events []connreject.Event
	e := &userspaceEngine{}
	e.SetConnRejectCallback(func(evt connreject.Event) { events = append(events, evt) })
	...
	if got := e.trackOpenPreFilterIn(&pp, nil); got != filter.Accept {
		t.Fatalf("filter response = %v, want Accept so tstun can handle mapping recovery", got)
	}
	if len(events) != 1 {
		t.Fatalf("got %d events, want 1", len(events))
	}
	if got := events[0].Reason; got != connreject.ReasonAppConnectorTransitIPUnknown {
		t.Errorf("Reason = %q, want %q", got, connreject.ReasonAppConnectorTransitIPUnknown)
	}
	if got := events[0].Source; got != connreject.SourceTSMPRecv {
		t.Errorf("Source = %q, want %q", got, connreject.SourceTSMPRecv)
	}
}
...
func TestRejectReasonToReason(t *testing.T) {
	...
	tests := []struct {
		...
		{packet.RejectedDueToUnknownAppConnectorTransitIP, connreject.ReasonAppConnectorTransitIPUnknown},
