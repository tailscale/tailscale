...
// the MTU go lower ude to very low-MTU IPv4 interfaces.
//
...
func getDefaultRouteMTU() (uint32, error) {
	...
	nonTSLUIDs := map[uint64]uint32{} // LUID -> MTU
	for _, iface := range ifs {
		if iface.Description != "Tailscale Tunnel" {
			nonTSLUIDs[iface.Luid] = iface.Mtu
		}
	}
	...
	for _, route := range routes {
		...
		routeMTU := nonTSLUIDs[route.InterfaceLuid]
		if routeMTU == 0 {
			continue
		}
