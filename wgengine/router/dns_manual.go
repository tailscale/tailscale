...
// dnsManualUp replaces /etc/resolv.conf with a file generated
// from the given nameservers and search domains, creating a backup of its old state.
//
// This way of configuring DNS is precarious, since it does not react
// to the disappearance of the Tailscale interface.
// The caller must call restoreResolvConf before program shutdown.
func dnsManualUp(servers []netaddr.IP, domains []string) error {
	...
	writeTsConf(buf, servers, domains)
...
// dnsManualDown restores /etc/resolv.conf to its state before replaceResolvConf.
// It is idempotent and behaves correctly even if replaceResolvConf has never been run.
func dnsManualDown() error {
