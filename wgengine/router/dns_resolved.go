...
// resolvedSetTimeout is the time interval within which
// all operations in resolvedUpDNS must complete.
//
// This is useful because certain conditions (such as improper dbus auth)
// can cause a context-less Call to hang indefinitely.
const resolvedSetTimeout = time.Second
...
// We consider this case as lacking resolved support and fall through to replaceResolvConf.
//
...
func dnsResolvedUp(servers []netaddr.IP, domains []string) error {
	ctx, cancel := context.WithTimeout(context.Background(), resolvedSetTimeout)
	defer cancel()
	...
	var linkDomains = make([]resolvedLinkDomain, len(domains))
