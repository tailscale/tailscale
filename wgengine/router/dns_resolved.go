...
// resolvedStubPath maps the locations of resolved resolv.conf stubs to true.
//
...
var resolvedStubPath = map[string]bool{
	"/lib/systemd/resolv.conf":                  true,
	"/usr/lib/systemd/resolv.conf":              true,
	"/run/systemd/resolve/stub-resolv.conf":     true,
	"/var/run/systemd/resolve/stub-resolv.conf": true,
}
...
func resolvedIsActive() bool {
	path, err := os.Readlink("/etc/resolv.conf")
	if err != nil {
		return false
	}

	return resolvedStubPath[path]
}
