...
func (m *winMon) Close() (ret error) {
	...
	if m.addressChangeCallback != nil {
		if err1 := m.addressChangeCallback.Unregister(); err1 != nil {
			m.logf("addressChangeCallback.Unregister error: %v", err1)
			ret = err1
		} else {
	...
	if m.routeChangeCallback != nil {
		if err1 := m.routeChangeCallback.Unregister(); err1 != nil {
			m.logf("routeChangeCallback.Unregister error: %v", err1)
			ret = err1
		} else {
