...
// BlueprintTagNamespacePrefix is the literal prefix (immediately after
// "tag:") for tags that are auto-generated by the control plane on behalf
// of a Blueprint. The double slash is intentional: it visually marks
// blueprint-managed tags so operators can distinguish them at a glance
// from user-authored tags. Blueprint-managed tags do not require a
// tagOwners entry (the control plane is the implicit owner).
const BlueprintTagNamespacePrefix = "bp//"
...
//
// The one exception is the "tag:bp//<id>" namespace, which is reserved for
// blueprint-managed tags. The <id> portion follows the same rules as a
// regular tag name (letter-start, alphanumerics and dashes).
//
...
func CheckTag(tag string) error {
	body, ok := strings.CutPrefix(tag, "tag:")
	if !ok {
		return errors.New("tags must start with 'tag:'")
	}
	if rest, isBP := strings.CutPrefix(body, BlueprintTagNamespacePrefix); isBP {
		return checkTagBody(rest, "blueprint tag IDs")
	}
	return checkTagBody(body, "tag names")
}

// checkTagBody validates the post-prefix portion of a tag name (whether
// the prefix was "tag:" or "tag:bp//"). errNoun names what's being
// validated for use in human-readable error messages (e.g. "tag names" or
// "blueprint tag IDs").
func checkTagBody(name, errNoun string) error {
	if name == "" {
		return fmt.Errorf("%s must not be empty", errNoun)
	}
	if !isAlpha(name[0]) {
		return fmt.Errorf("%s must start with a letter", errNoun)
	}
	for _, b := range []byte(name) {
		if !isNum(b) && !isAlpha(b) && b != '-' {
			return fmt.Errorf("%s can only contain numbers, letters, or dashes", errNoun)
		}
	}
	return nil
}

// IsBlueprintTag reports whether tag is a syntactically valid
// blueprint-managed tag of the form "tag:bp//<id>". It does not check
// whether the referenced blueprint exists.
func IsBlueprintTag(tag string) bool {
	rest, ok := strings.CutPrefix(tag, "tag:"+BlueprintTagNamespacePrefix)
	if !ok {
		return false
	}
	return checkTagBody(rest, "blueprint tag IDs") == nil
}

// BlueprintIDFromTag returns the <id> portion of a blueprint-managed tag
// "tag:bp//<id>". If tag is not a syntactically valid blueprint tag, it
// returns "" and false.
func BlueprintIDFromTag(tag string) (id string, ok bool) {
	rest, ok := strings.CutPrefix(tag, "tag:"+BlueprintTagNamespacePrefix)
	if !ok {
		return "", false
	}
	if checkTagBody(rest, "blueprint tag IDs") != nil {
		return "", false
	}
	return rest, true
}
