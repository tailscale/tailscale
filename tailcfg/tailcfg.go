...
//   - 114: 2025-01-29: Client sends audit-log entries to control
...
const (
	...
	// NodeAttrAuditLogs grants a peer the ability to send audit logs via controlclient
...
// ClientAuditAction represents an aduitable action that a client can report to the
// control plane.  These actions must correspond to the supported actions
// in the control plane.
type ClientAuditAction string

const (
	// AuditNodeDisconnect action should be sent when a node has disconnected
	// from the control plane.  The details should include a "reason" field
	// with a string describing the reason, potentially supplied by the user.
	AuditNodeDisconnect = ClientAuditAction("DISCONNECT_NODE")
)

// ClientAuditLog represents a pending audit log request generated by a client.
//
// EventID, ProfileID and Retries are used to track the status of the audit log
// and whether or not it may be retried.
//
// The Action and Details correspond the the Action and Details fields that will
// written to the audit log request to the control plane.
type ClientAuditLog struct {
	// A random, unique ID for the audit log request.
	EventID string `json:",omitempty"`
	// The fixed, unique userProfile ID to whom the audit log is associated.
	ProfileID string `json:",omitempty"`
	// Retries is the number of times we've attempted to submit this log.
	Retries int `json:",omitempty"`

	// The action must correspond to a known action in the control plane.
	Action ClientAuditAction `json:",omitempty"`
	// The details will be use to populate ActionDetails  in the audit
	// log request.
	Details string `json:",omitempty"`
}

// AuditLogRequest represents an audit log request to be sent to the control
// Details is an opaque string specific to the action being audited.
