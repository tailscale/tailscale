...
func init() {
	...
	taildrive.DisallowShareAs = true
}
...
func TestDirectoryListing(t *testing.T) {
	...
	s.addShare(remote1, share11, taildrive.PermissionReadWrite)
	s.checkDirList("remote with one share should contain that share", shared.Join(domain, remote1), share11)
	s.addShare(remote1, share12, taildrive.PermissionReadOnly)
...
type remote struct {
	...
	permissions map[string]taildrive.Permission
	mu          sync.RWMutex
}
...
func (s *system) addRemote(name string) {
	...
	r := &remote{
		...
		permissions: make(map[string]taildrive.Permission),
	}
	r.fs.SetFileServerAddr(fileServer.Addr())
	go http.Serve(l, r)
	s.remotes[name] = r

	remotes := make([]*taildrive.Remote, 0, len(s.remotes))
	for name, r := range s.remotes {
		remotes = append(remotes, &taildrive.Remote{
			Name: name,
...
func (s *system) addShare(remoteName, shareName string, permission taildrive.Permission) {
	...
	shares := make([]*taildrive.Share, 0, len(r.shares))
	for shareName, folder := range r.shares {
		shares = append(shares, &taildrive.Share{
			Name: shareName,
			Path: folder,
		})
	}
